...
const Analytics: React.FC = () => {
    ...
    const [bundlingData, setBundlingData] = useState<any[]>([]);
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const bundlingRes = await axios.get(`${API_URL}/stats/insights/bundling`, config);
            setBundlingData(bundlingRes.data || []);

            const inactiveRes = await axios.get(`${API_URL}/stats/insights/inactive-customers`, config);
            setInactiveCustomers(inactiveRes.data || []);

            const slowMovingRes = await axios.get(`${API_URL}/stats/insights/slow-moving`, config);
            setSlowMovingStock(slowMovingRes.data || []);

            const peakRes = await axios.get(`${API_URL}/stats/insights/peak-times`, config);
            setPeakTimes(peakRes.data || []);

            const segmentsRes = await axios.get(`${API_URL}/stats/insights/customer-segments`, config);
